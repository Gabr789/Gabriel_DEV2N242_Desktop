...
class TelaListaUsuarios:
    def __init__(self, master=None):
        ...
        self.treeview = ttk.Treeview(self.frame_conteudo, columns=("Usuário"), show="headings")
    ...
    def carregar_usuarios(self):
        try:
            ...
            for usuario in usuarios:
                self.treeview.insert("", "end", values=(usuario[0],))
    ...
    def abrir_edicao(self):
        ...
        def salvar_edicao():
            ...
            try:
                ...
                query_update = "UPDATE usuarios SET usuario = %s, senha = %s WHERE usuario = %s"
                self.banco.cursor.execute(query_update, (novo_usuario, nova_senha, usuario))
                self.banco.conexao.commit()

                messagebox.showinfo("Sucesso", f"Usuário '{novo_usuario}' atualizado com sucesso!")
                edicao.destroy()  # Fecha a janela de edição
